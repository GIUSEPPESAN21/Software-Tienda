...
# PÁGINAS (SIN CAMBIOS)
# ----------------------------------
if st.session_state.page not in ["📊 Analítica"]:
    # El código para las otras páginas se mantiene igual al que proporcionaste.
    # ... (Aquí iría todo el código de las páginas Inicio, Análisis IA, Inventario, etc.)
    # Por brevedad, se omite y nos centramos en la página de Analítica.
...
# ----------------------------------
# PÁGINA: ANALÍTICA (VERSIÓN MEJORADA)
# ----------------------------------
    ...
    if not completed_orders:
        ...
        with tab1:
            ...
            for order in completed_orders:
                ...
                sales_data.append({
                    'Fecha': order['timestamp_obj'].date(),
                    'Ingresos': order.get('price', 0),
                    'Beneficios': order_profit
                })
        ...
        with tab2:
            ...
            item_profits = {}
            for item in all_items_sold:
                profit = (item.get('sale_price', item.get('purchase_price')) - item.get('purchase_price', 0)) * item.get('quantity', 0)
